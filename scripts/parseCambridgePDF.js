...
const input = {
  remote: new URL(
    'images/506887-b1-preliminary-2020-vocabulary-list.pdf',
...
function parseWords(items, words) {
  for (const item of items) {
    ...
    if (item.y >= 8 && item.y <= 79) {
      // skip header and footer
      if (text) {
        if (text !== ' ' && !/^[A-Z]$/.test(text)) {
          if ([6, 7, 32, 33].includes(item.x)) {
            // 6,7 or 32,33
            words.push(text)
          } else {
            //
  ...
  for (const item of items) {
    ...
    if (item.y >= 8 && item.y <= 79) {
      ...
      if (text) {
        if (/^[A-Z]/.test(text)) {
          let fixedTopic = text
            .replace(/^Places:$/, 'Places: Countryside')
            .replace(/\s\(Adjectives\)$/, '')
